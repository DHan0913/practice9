...
public class RecListApp {
	...
	public static void main(String[] args) {
		...
		Iterator<Rectangle> it = rList.iterator(); // 반복자 추출

		while (it.hasNext()) { // 뒤에 더 있나?
			Rectangle item = it.next(); // 요소 추출 후 다음으로 리턴
			item.draw();
		}
		...
		rList.remove(2);
		// 리스트 모두를 draw하는 메소드 실행
		Iterator<Rectangle> st = rList.iterator(); // 반복자 추출

		while (st.hasNext()) { // 뒤에 더 있나?
			Rectangle item = st.next(); // 요소 추출 후 다음으로 리턴
			item.draw();
		}
		...
		r04.setWidth(9);
		// 리스트 모두를 draw하는 메소드 실행
		Iterator<Rectangle> vt = rList.iterator(); // 반복자 추출

		while (vt.hasNext()) { // 뒤에 더 있나?
			Rectangle item = vt.next(); // 요소 추출 후 다음으로 리턴
			item.draw();
		}
